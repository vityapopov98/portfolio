<template>
  <div class="project-page">
    ...
    <div class="project-banner">
      ...
      <img :src="projectDetails.headerImg" alt="" class="project-banner-img" />
    </div>
    <div class="container-p">
      <div
        class="row"
        v-for="(element, index) in projectDetails.text"
        :key="index"
      >
        <div class="col-md-12">
          <!-- <h2>{{projectName}}</h2> -->
          <img
            v-if="element.includes('pic:')"
            :src="require(`@/assets/${element.slice(4)}`)"
            alt=""
            class="project-img"
          />
          <img
            v-else-if="element.includes('img:')"
            :src="element.slice(4)"
            alt=""
            class="project-img"
          />
          <audio
            v-else-if="element.includes('audio:')"
            controls
            :src="element.slice(6)"
          >
    ...
    loadProject() {
      ...
      this.$root.projects.forEach((element) => {
        ...
        if (element.urlName == this.projectName) {
          ...
          // Подготовить прямые ссылки на файлы.

          const article = element.text;
          article.forEach((paragraph, index) => {
            if (paragraph.includes("wdLink:")) {
              console.log(
                "fethcing link",
                paragraph.substring(
                  paragraph.indexOf(":") + 1,
              ...
              fetch(
                ...
                // распарсить страницу
                console.log("wd anser", res);
                console.log(res.body);
                article[index] = "wdLink:" + "новая ссылка";
              });
            }
          });
